...
print "<script language=\"JavaScript\" src=\"", $conf{'htmlRoot'}, "/js/reslist-tmp.js\"></script>\n";
...
#	    $source_line.=sprintf("'mywindow','height=400,width=1000,status,scrollbars,resizable,screenX=0,screenY=5');\">\n<img style='border-style:none' src='http://omilia.uio.no/glossa/html/img/sound.gif'>\n</a> \n&nbsp;</font>");
	...
	print_tokens($res_l, $atttype);
	if ($context_type eq "chars") {$source_line.=sprintf("</td><td>"); }
	$source_line.=sprintf("<b> &nbsp;");
	print_tokens($ord, $atttype);
	$source_line.=sprintf(" &nbsp;</b>");
	if ($context_type eq "chars") { $source_line.=sprintf("</td><td>"); }
	print_tokens($res_r, $atttype);
...
print "<script language=\"javascript\">showList($d_files,'".$conf{'query_id'}."','".$lang{'hits_found'}."',$hits,'".$lang{'results_pages'}."','$CORPUS','$max')</script>\n";
