...
my $cnt_select = 0;
if ($CORPUS =~ /^(skriv|norm)$/) {
  my $word_count = $dbh->selectrow_array("SELECT MAX(endpos)+1 FROM $text_table");
...
foreach my $tid (@tids_sorted) {
    ...
    if ($cnt_select) {
        ...
        print "<td>$word_count</td>\n";
            ...
            my $assignment_path = "/$CORPUS/oppgavetekster/${assignment_code}.pdf";
	    ...
	    $source_line.=sprintf("'mywindow','height=600,width=600,status,scrollbars,resizable');\"><img src='$conf{'htmlRoot'}/html/img/i.gif' alt='i' border='0'></a>&nbsp;</font>");
            ...
            if (-e "/var/www/html$assignment_path") {
                $source_line.=sprintf("<a href=\"$assignment_path\" target=\"_new\"><img src=\"/$CORPUS/img/assignment-text.png\" height=\"14\"/></a>&nbsp;");
            }
            ...
            if (-e "/var/www/html$answer_path") {
                $source_line.=sprintf("<a href=\"$answer_path\" target=\"_new\"><img src=\"/$CORPUS/img/assignment-answer.png\" height=\"14\"/></a>");
            }
